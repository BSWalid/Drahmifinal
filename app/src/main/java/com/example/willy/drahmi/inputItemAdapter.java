...
public class inputItemAdapter extends RecyclerView.Adapter<inputItemAdapter.MyViewBolder> {
    List <inputitem> items;
    Context context;

    public inputItemAdapter(List<inputitem> items, Context context) {
        this.items = items;
        this.context = context;
    }

    @Override
    public inputItemAdapter.MyViewBolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view;
        view = LayoutInflater.from(parent.getContext()).inflate(R.layout.iteminput,parent,false);
        return new inputItemAdapter.MyViewBolder(view);
    }



    @Override
    public void onBindViewHolder(inputItemAdapter.MyViewBolder holder, int position) {
